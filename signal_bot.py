...
def format_alert(ticker: str, row) -> str:
...
def format_daily_summary(ticker: str, row) -> str:
    patterns = [name for name in
                ["doji", "hammer", "shooting_star", "bullish_engulfing", "bearish_engulfing"]
                if row.get(name, False)]
    lines = [
...
def run_once(args, token, chat_id):
    ...
    last = df.iloc[-1]
    if args.notify and last["signal"] != "HOLD":
        if token and chat_id:
            ok = send_telegram(token, chat_id, format_alert(args.ticker, last))
    ...
    while True:
        try:
            ...
            if args.daily and prev.get("last_daily_sent") != today:
                ok = send_telegram(token, chat_id, format_daily_summary(args.ticker, last))
