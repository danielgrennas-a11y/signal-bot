...
WHAT THIS IS
    ...
    buy and sell SIGNALS based on those rules. It also pulls recent Yahoo
    Finance headlines for the ticker and scores them with a simple
    positive/negative keyword count, then combines that with the technical
    score into one final verdict. In --watch mode it polls periodically and
    pings your phone via Telegram when the signal changes.

    The news scoring is a plain keyword counter, NOT real NLP or an LLM
    reading of the news — it counts words like "surge"/"plunge" in
    headlines. It's a rough steer, easily fooled by sarcasm, single-company
    stories mixed in with market-wide ones, or wording it doesn't
    recognize. Disable it any time with --no-news to fall back to pure
    technical signals.
...
# --------------------------------------------------------------------------
# News sentiment (headline keyword scoring — deliberately simple/transparent)
# --------------------------------------------------------------------------
# This is NOT real NLP or an LLM read of the news — it's a plain keyword
# count on headlines pulled from Yahoo Finance. Treat the result as a rough
# steer, not an authoritative "the news says X". Headlines can be sarcastic,
# about a single company rather than the whole index, or simply not covered
# by this word list — all of that is invisible to a keyword counter.

POSITIVE_WORDS = [
    "surge", "surges", "jump", "jumps", "rally", "rallies", "gain", "gains",
    "rise", "rises", "rising", "record", "beat", "beats", "upgrade", "upgraded",
    "strong", "growth", "profit", "profits", "bullish", "outperform",
    "recovery", "boost", "soar", "soars", "optimis", "rebound",
]
NEGATIVE_WORDS = [
    "plunge", "plunges", "plunging", "fall", "falls", "falling", "drop", "drops",
    "slump", "crash", "crashes", "downgrade", "downgraded", "weak", "loss",
    "losses", "bearish", "underperform", "recession", "warning", "warns",
    "cut", "cuts", "layoff", "layoffs", "bankrupt", "bankruptcy", "sell-off",
    "selloff", "tumble", "tumbles", "slide", "slides",
]
...
def score_headline(title: str) -> int:
    words = set(w.lower() for w in _word_re.findall(title))
    pos = sum(1 for w in POSITIVE_WORDS if any(w in tok for tok in words))
    neg = sum(1 for w in NEGATIVE_WORDS if any(w in tok for tok in words))
    return pos - neg


def fetch_news(ticker: str, max_items: int = 8):
    """
    Pull recent headlines for this ticker via yfinance (Yahoo Finance).
    Returns a list of {"title", "publisher", "link", "score"} dicts.
    Returns an empty list on any failure — news is a nice-to-have, never
    a reason to crash the whole run.
    """
    try:
        import yfinance as yf
    ...
    for entry in raw[:max_items]:
        ...
        link = (content.get("canonicalUrl") or {}).get("url") if isinstance(content.get("canonicalUrl"), dict) \
            else entry.get("link")
        if not title:
            continue
        items.append({
...
def aggregate_news_sentiment(items):
    """
    Turn a list of scored headlines into a single -1 / 0 / +1 contribution
    to the overall signal score, plus a human-readable label. Deliberately
    capped at +/-1 so news can nudge a technical HOLD into BUY/SELL territory
    but can never single-handedly override strong technical signals.
    """
    if not items:
        return 0, "no news data", items
    total = sum(i["score"] for i in items)
    if total >= 2:
        return 1, "leaning positive", items
    elif total <= -2:
        return -1, "leaning negative", items
    else:
        return 0, "mixed/neutral", items


def build_news_verdict(ticker, row):
    """
    Fetch news, score it, and combine with the technical score to produce
    a combined signal. Never raises — any failure just yields no news info,
    and the caller falls back to the pure technical signal.
    """
    items = fetch_news(ticker)
    contribution, label, items = aggregate_news_sentiment(items)
    tech_score = int(row["score"])
    combined_score = tech_score + contribution
    combined_signal = "BUY" if combined_score >= 2 else "SELL" if combined_score <= -2 else "HOLD"
    return {
        "items": items,
...
def format_alert(ticker: str, df: pd.DataFrame, row, news=None) -> str:
    ...
    if news is not None:
        lines.append("")
        lines.append(f"Technical score: {int(row['score'])}  |  News: {news['label']} ({news['contribution']:+d})  |  Combined: {news['combined_score']:+d}")
        for item in news["items"][:3]:
            lines.append(f"  • {item['title']} ({item['publisher']})")

    lines.append("(Technical + headline-keyword signal — not financial advice.)")
    return "\n".join(lines)


def format_daily_summary(ticker: str, df: pd.DataFrame, row, news=None) -> str:
    patterns = [name for name in
                ["doji", "hammer", "shooting_star", "bullish_engulfing", "bearish_engulfing"]
                if row.get(name, False)]
    lines = [
    ...
    if news is not None:
        lines.append("")
        lines.append(f"News sentiment: {news['label']} ({news['contribution']:+d})")
        lines.append(f"Combined score: {news['combined_score']:+d}  ->  {news['combined_signal']}")
        for item in news["items"][:3]:
            lines.append(f"  • {item['title']} ({item['publisher']})")
...
def run_once(args, token, chat_id):
    ...
    news = None
    if not args.no_news:
        news = build_news_verdict(args.ticker, last)
        print(f"News: {news['label']} (contribution {news['contribution']:+d}) "
              f"| technical {news['tech_score']:+d} -> combined {news['combined_score']:+d} "
              f"-> {news['combined_signal']}")

    effective_signal = news["combined_signal"] if news else last["signal"]
    ...
    if args.notify:
        if not (token and chat_id):
            ...
            # itself is only scheduled once a day, so no dedup needed. Still
            # record state so the hourly runs know the latest signal too,
            # and so the state file always exists to be committed back.
            ok = send_telegram(token, chat_id, format_daily_summary(args.ticker, df, last, news=news))
            print("Daily summary sent." if ok else "Daily summary failed — check token/chat id.")
            state = load_state()
            state[key] = {"timestamp": last_ts, "signal": effective_signal}
            save_state(state)
        elif effective_signal != "HOLD":
            ...
            state = load_state()
            prev = state.get(key, {})
            is_new_signal = (prev.get("timestamp") != last_ts or prev.get("signal") != effective_signal)
            if is_new_signal:
                alert_row = dict(last)
                alert_row["signal"] = effective_signal  # reflect the combined verdict in the message
                ok = send_telegram(token, chat_id, format_alert(args.ticker, df, alert_row, news=news))
                print("Telegram alert sent." if ok else "Telegram alert failed — check token/chat id.")
            else:
                print(f"Signal is still {effective_signal} from a previous run — not re-sending.")
            # Record state regardless, so the file always exists and hourly
            # HOLD runs (below) still update the "last checked" timestamp.
            state[key] = {"timestamp": last_ts, "signal": effective_signal}
            save_state(state)
        else:
            # HOLD and not daily mode: still touch the state file so it
            # exists for the commit step, without changing the recorded signal.
            state = load_state()
    ...
    while True:
        try:
            ...
            print(f"[{pd.Timestamp.now().strftime('%H:%M:%S')}] "
                  ...
                  + (f" news={news['label']} combined={effective_signal}" if news else ""))
            ...
            is_new_signal = (
                ...
                and (prev.get("timestamp") != last_ts or prev.get("signal") != effective_signal)
            )
            if is_new_signal:
                alert_row = dict(last)
                alert_row["signal"] = effective_signal
                ok = send_telegram(token, chat_id, format_alert(args.ticker, df, alert_row, news=news))
                print("  -> Signal alert sent." if ok else "  -> Signal alert failed.")
                prev["timestamp"] = last_ts
                prev["signal"] = effective_signal
                state[key] = prev
                save_state(state)
...
def main():
    ...
    parser.add_argument("--no-news", action="store_true",
                         help="Skip fetching/scoring news headlines — use pure technical signal only")
